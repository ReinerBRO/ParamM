...
def get_args():
    ...
    parser.add_argument("--phase2_search_mode", type=str, choices=["linear", "search"], default="linear",
                        help="Phase2 mode: `linear` (default) or `search`")
...
def strategy_factory(strategy: str):
    ...
    if strategy == "simple":
        return kwargs_wrapper_gen(run_simple, delete_keys=["expansion_factor", "phase1_only", "global_mem_bank_path", "phase2_search_mode"])
    elif strategy == "dot":
        return kwargs_wrapper_gen(run_dot, delete_keys=["expansion_factor"])
    elif strategy == "dot_bank":
        return kwargs_wrapper_gen(run_dot_bank, delete_keys=["expansion_factor", "phase1_only", "global_mem_bank_path", "phase2_search_mode"])
    elif strategy == "reflexion":
        return kwargs_wrapper_gen(run_reflexion, delete_keys=["expansion_factor", "phase1_only", "global_mem_bank_path", "phase2_search_mode"])
    elif strategy == "test-acc":
        return kwargs_wrapper_gen(run_test_acc, delete_keys=["expansion_factor", "max_iters", "phase1_only", "global_mem_bank_path", "phase2_search_mode"])
    else:
...
""")
    ...
    if args.strategy != 'simple' and 'mbpp' not in args.dataset_path.lower():
        run_strategy(
            ...
            phase2_search_mode=args.phase2_search_mode
        )
    else:
        run_strategy(
            dataset=dataset,
