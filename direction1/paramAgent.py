...
def _feedback_to_pass_rate(feedback: str, tests_i: list) -> float:
    if not tests_i:
        return 0.0
    try:
        failed_count = feedback.split("Tests failed:")[1].count("assert")
    except Exception:
        failed_count = len(tests_i)
    passed_count = max(len(tests_i) - failed_count, 0)
    return passed_count / max(len(tests_i), 1)
...
def _run_phase2_search(
    ...
    implementations = [cur_func_impl]
    test_feedback = [cur_feedback]
    visible_feedback_cache: Dict[str, str] = {}

    def _get_cached_visible_feedback(code: str) -> str:
        cached_feedback = visible_feedback_cache.get(code)
        if cached_feedback is None:
            cached_feedback = _visible_verifier(code).details
            visible_feedback_cache[code] = cached_feedback
        return cached_feedback

    def _visible_verifier(code: str) -> VerificationResult:
        if dataset_type == 'livecodebench':
            is_passing_local, local_feedback = _evaluate_with_feedback_livecodebench(
                exe, identifier, code, tests_i, timeout=30
            )
            pass_rate = local_feedback.count("PASS") / max(len(tests_i), 1) if tests_i else 0.0
            return VerificationResult(
                passed=is_passing_local,
                pass_rate=pass_rate,
                cost=1.0,
                details=local_feedback,
            )

        is_passing_local, local_feedback, _ = exe.execute(code, tests_i)
        pass_rate = _feedback_to_pass_rate(local_feedback, tests_i)
        return VerificationResult(
            passed=is_passing_local,
            pass_rate=pass_rate,
            cost=1.0,
    ...
    def _generated_verifier(code: str) -> VerificationResult:
        if dataset_type == 'livecodebench':
            passed_eval = exe.evaluate_livecodebench(identifier, code, private_tests, timeout=30)
        else:
            passed_eval = exe.evaluate(identifier, code, test_code, timeout=10)
        return VerificationResult(
            passed=passed_eval,
            pass_rate=1.0 if passed_eval else 0.0,
            cost=1.0,
            details="evaluate",
        )

    def _static_verifier(code: str) -> VerificationResult:
        try:
            compile(code, "<phase2_search_candidate>", "exec")
            return VerificationResult(passed=True, pass_rate=1.0, cost=0.0, details="syntax-ok")
        except Exception as e:
            return VerificationResult(passed=False, pass_rate=0.0, cost=0.0, details=str(e))

    def _generate_candidates(parent: SearchNode | None = None, **_: Any) -> list:
        source_impl = cur_func_impl if parent is None else parent.code
        if parent is None:
            source_feedback = cur_feedback
        else:
            source_feedback = parent.metadata.get("visible_feedback")
            if source_feedback is None:
                source_feedback = _get_cached_visible_feedback(source_impl)

        if use_mistakes:
            if pitfall_agent is not None:
                candidate_insights = pitfall_agent.generate(prompt, temperature=1.0)
            elif (
                mistake_json_file and
                ...
                idx = min((parent.depth if parent else 0), len(lst) - 1)
                candidate_insights = mistake_json_file[original_idx]['high_temp_pitfall'][lst[idx]]
            else:
                candidate_insights = ""
        else:
            candidate_insights = ""

        reflections = gen.self_reflection_diverse_oneshot_parametric(
            source_impl,
            source_feedback,
            model,
            diverse_reflections,
        ...
        reflections = [ref for ref in reflections if len(ref) > 10][:2]
        ...
        for reflection in reflections:
            new_func_impl = gen.func_impl(
                ...
                model=model,
                strategy="reflexion",
                prev_func_impl=source_impl,
                feedback=source_feedback,
                self_reflection=reflection,
                temperature=1.0,
                ref_chat_instruction='dot',
                mistake_insights=candidate_insights,
            )
            if not isinstance(new_func_impl, str):
                continue
            diverse_reflections.append(reflection)
            child_feedback = _get_cached_visible_feedback(new_func_impl)
            children.append(
                SearchNode(
                    code=new_func_impl,
                    reflection=reflection,
                    metadata={"visible_feedback": child_feedback},
                )
            )
    ...
    search_engine = SearchEngine(
        config=SearchConfig(
            beam_size=2,
            max_nodes=max(2, max_iters * 3),
            max_depth=max(1, max_iters),
            early_stop=True,
        ),
        candidate_generator=CandidateGeneratorAdapter(_generate_candidates),
        verifier=VerifierAdapter(
            ...
            visible_tests_fn=_visible_verifier,
            generated_tests_fn=_generated_verifier,
            static_checks_fn=_static_verifier,
        ),
    )

    initial_node = SearchNode(code=cur_func_impl, reflection="phase2-initial")
    search_result = search_engine.run(initial_nodes=[initial_node])

    best_node = search_result.best_node
    if best_node is not None and best_node.code not in implementations:
        implementations.append(best_node.code)

    final_solution = cur_func_impl if best_node is None else best_node.code
    final_feedback = cur_feedback
    final_is_solved = False

    if dataset_type == 'livecodebench':
        _, final_feedback = _evaluate_with_feedback_livecodebench(
            exe, identifier, final_solution, tests_i, timeout=30
        )
        final_is_solved = exe.evaluate_livecodebench(identifier, final_solution, private_tests, timeout=30)
    else:
        _, final_feedback, _ = exe.execute(final_solution, tests_i)
        final_is_solved = exe.evaluate(identifier, final_solution, test_code, timeout=10)

    test_feedback.append(final_feedback)

    return {
        "solution": final_solution,
        "is_solved": final_is_solved,
        "implementations": implementations,
        "test_feedback": test_feedback,
        "search_nodes_expanded": search_result.nodes_expanded,
        "verifier_cost": float(search_result.verifier_cost),
        "best_score_trace": list(search_result.best_score_trace),
    }
...
def run_dot(
    ...
    if phase2_search_mode not in {"linear", "search"}:
        print(f"[WARN] Unknown phase2_search_mode={phase2_search_mode}, fallback to linear")
        phase2_search_mode = "linear"
    ...
    # Ensure second-stage observability fields exist for every record,
    # including samples that do not enter phase2 retry.
    for rec in logs_copy:
        rec.setdefault("phase2_search_mode", phase2_search_mode)
        rec.setdefault("search_nodes_expanded", 0)
        rec.setdefault("verifier_cost", 0.0)
        rec.setdefault("best_score_trace", [])
        rec.setdefault("search_fallback_to_linear", False)

    if not failed_problems:
        write_jsonl(second_stage_json, logs_copy, append=False)
        print(f"[info] No failed problems; second-stage log saved to: {second_stage_json} (n={len(logs_copy)})")
        return
    ...
    for i, item in enumerate(failed_problems):
        ...
        try:
            ...
            # Phase2 search metadata defaults (required for both linear and search)
            item["phase2_search_mode"] = phase2_search_mode
            item["search_nodes_expanded"] = 0
            item["verifier_cost"] = 0.0
            item["best_score_trace"] = []
            item["search_fallback_to_linear"] = False
            ...
            while cur_pass < pass_at_k and not is_solved:
                ...
                curr_emb_col = curr_emb[:, None] if curr_emb is not None else None
                retrieval_pool = global_positive_trajectories + memory_bank["positive_trajectories"]
                if len(retrieval_pool) > 0 and curr_emb_col is not None:
                ...
                if phase2_search_mode == "search":
                    try:
                        search_output = _run_phase2_search(
                            item=item,
                            prompt=prompt,
                            ...
                            diverse_reflections=diverse_reflections,
                        )

                        cur_func_impl = search_output["solution"]
                        is_solved = search_output["is_solved"]
                        implementations = implementations + search_output["implementations"]
                        test_feedback = test_feedback + search_output["test_feedback"]
                        if search_output["test_feedback"]:
                            cur_feedback = search_output["test_feedback"][-1]
                        item["search_nodes_expanded"] = int(search_output["search_nodes_expanded"])
                        item["verifier_cost"] = float(search_output["verifier_cost"])
                        item["best_score_trace"] = list(search_output["best_score_trace"])
                        ...
                        item["search_fallback_to_linear"] = True
                        print(f"[WARN] Phase2 search failed, fallback to linear: {search_error}")
        ...
        for log_item in logs_copy:
            if log_item.get(primary_key) == item.get(primary_key):
                ...
                log_item["phase2_search_mode"] = item.get("phase2_search_mode", "linear")
                log_item["search_nodes_expanded"] = int(item.get("search_nodes_expanded", 0))
                log_item["verifier_cost"] = float(item.get("verifier_cost", 0.0))
                log_item["best_score_trace"] = list(item.get("best_score_trace", []))
                log_item["search_fallback_to_linear"] = bool(item.get("search_fallback_to_linear", False))
