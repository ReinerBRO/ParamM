...
class PyExecutor(Executor):
    def execute(self, func: str, tests: List[str], timeout: int = 5) -> ExecuteResult:
        ...
        for i in range(num_tests):
            try:
                # Use a shared globals/locals namespace so nested test helpers
                # (e.g., MBPP's test_check -> check) resolve correctly.
                scope = {"__builtins__": __builtins__}
                function_with_timeout(exec, (func_test_list[i], scope, scope), timeout)
...
check({name})
    """
        try:
            # Shared globals/locals is required for MBPP tests where
            # test_check() calls check() defined in the same snippet.
            scope = {"__builtins__": __builtins__}
            function_with_timeout(exec, (code, scope, scope), timeout)
            return True
...
def get_output(func: str, assert_statement: str, timeout: int = 5) -> str:
    try:
        # Keep one shared scope for consistency with evaluate/execute.
        scope = {"__builtins__": __builtins__}
        exec(f"from typing import *\n{func}", scope, scope)
        func_call = get_call_str(assert_statement)
        output = function_with_timeout(eval, (func_call, scope, scope), timeout)
        return output
    except TimeoutError:
        return "TIMEOUT"
    except Exception as e:
