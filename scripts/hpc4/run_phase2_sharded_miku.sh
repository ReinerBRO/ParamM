...
ENV_NAME="${1:-paramm}"
RUN_NAME="${2:-llama3_1_8b_humaneval_20260302_112403}"
...
for w in $(seq 0 $((NUM_WORKERS - 1))); do
  ...
  ds_shard="${SHARDS_DIR}/humaneval_full_shard_$(printf "%02d" "${w}").jsonl"
  pf_shard="${SHARDS_DIR}/humaneval_full_pitfalls_shard_$(printf "%02d" "${w}").jsonl"
...
echo ""
echo "Phase2 sharded run started."
