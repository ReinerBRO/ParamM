...
def run_dot(
    ...
    def select_stage2_traj_ensemble(
        ...
        strategy: str = "prompt_sim",
    ) -> Optional[dict]:
        """
        INNOVATION: Multi-strategy retrieval (no broken router).
        
        Strategies:
            - "none": No retrieval (pure reflexion baseline)
            - "prompt_sim": Prompt similarity (current fallback)
            - "feedback_guided": Find examples that fixed similar test failures
        """
        if len(retrieval_pool) == 0 or strategy == "none":
            return None
        ...
        if strategy == "prompt_sim":
            # Strategy B: Prompt similarity
            return max(
                retrieval_pool,
                key=lambda traj: _safe_dot_sim(traj.get("prompt_embedding"), prompt_query),
            )
        
        elif strategy == "feedback_guided":
            # Strategy C: Feedback-guided retrieval
            ...
            error_keywords = []
            if "assert" in recent_feedback:
                error_keywords.append("assert")
            if "type" in recent_feedback or "attribute" in recent_feedback:
                error_keywords.append("type")
            if "index" in recent_feedback or "key" in recent_feedback:
                error_keywords.append("index")
            if "timeout" in recent_feedback or "recursion" in recent_feedback:
                error_keywords.append("timeout")
            if "value" in recent_feedback or "zero" in recent_feedback:
                error_keywords.append("value")
            
            # Score by feedback pattern + prompt similarity
            scored = []
            for traj in retrieval_pool:
                traj_feedback = str(traj.get("test_feedback", "")).lower()
                feedback_score = sum(1 for kw in error_keywords if kw in traj_feedback)
                prompt_score = _safe_dot_sim(traj.get("prompt_embedding"), prompt_query)
                # 60% prompt, 40% feedback pattern
                combined = 0.6 * prompt_score + 0.4 * (feedback_score / max(len(error_keywords), 1))
                scored.append((combined, traj))
    ...
    for i, item in enumerate(failed_problems):
        ...
        try:
            ...
            while cur_pass < pass_at_k and not is_solved:
                ...
                # ENSEMBLE: Try 3 strategies and pick best by public test feedback
                strategies = ["none", "prompt_sim", "feedback_guided"]
                strategy_trajs = []
                for strat in strategies:
                    traj = select_stage2_traj_ensemble(
                    ...
                    strategy_trajs.append((strat, traj))
                
                # For now, use prompt_sim as default (will ensemble after generation)
                initial_traj = strategy_trajs[1][1]  # prompt_sim
                router_mix = [0.0, 0.0, 0.0]
                router_conf = 0.0
                fallback_flag = True
                ...
                # INNOVATION: 3-way ensemble generation
                # Generate from 3 strategies and pick best by public test feedback
                ensemble_candidates = []
                ensemble_feedbacks = []
                ensemble_strategies = []
                
                for strat_name, strat_traj in strategy_trajs:
                    # Build prompt for this strategy
                    if strat_traj is not None:
                        strategy_examples = [strat_traj] + [x for x in closest if x is not strat_traj]
                        strategy_examples = strategy_examples[:3]
                    else:
                        strategy_examples = closest[:2] if strat_name != "none" else []
                    
                    if strategy_examples:
                        strategy_prompt = _build_augmented_prompt_from_examples(strategy_examples, prompt)
                    else:
                    ...
                    if use_mistakes:
                        if pitfall_agent is not None:
                            strategy_insights = pitfall_agent.generate(prompt, temperature=0.1)
                        else:
                            if strategy_examples and "mistake_insights" in strategy_examples[0]:
                                strategy_insights = strategy_examples[0]["mistake_insights"]
                            elif 'mistake_insights' in item:
                                strategy_insights = item['mistake_insights']
                            else:
                                strategy_insights = ""
                    else:
                        strategy_insights = ""
                    ...
                    # Generate solution
                    strategy_impl = gen.func_impl(
                        strategy_prompt,
                        model,
                        "simple",
                        temperature=1.0,
                        mistake_insights=strategy_insights
                    )
                    
                    if not isinstance(strategy_impl, str):
                        continue
                    
                    # Test on public tests
                    if dataset_type == 'livecodebench':
                        strat_passing, strat_feedback = _evaluate_with_feedback_livecodebench(
                            exe, identifier, strategy_impl, tests_i, timeout=30
                        )
                    else:
                        strat_passing, strat_feedback, _ = exe.execute(strategy_impl, tests_i)
                    
                    ensemble_candidates.append(strategy_impl)
                    ensemble_feedbacks.append(strat_feedback)
                    ensemble_strategies.append(strat_name)
                    implementations.append(strategy_impl)
                    test_feedback.append(strat_feedback)
                    
                    print(f"[ENSEMBLE] Strategy={strat_name}, passing={strat_passing}, feedback={strat_feedback[:100]}")
                
                # Pick best candidate by public test feedback
                if ensemble_candidates:
                    best_idx = _select_candidate_index_by_feedback(
                        [fb.startswith("All tests passed") or "PASS" in fb for fb in ensemble_feedbacks],
                        ensemble_feedbacks,
                    )
                    cur_func_impl = ensemble_candidates[best_idx]
                    cur_feedback = ensemble_feedbacks[best_idx]
                    best_strategy = ensemble_strategies[best_idx]
                    print(f"[ENSEMBLE] Selected strategy={best_strategy}, idx={best_idx}")
                    
                    # Use the trajectory from best strategy for refined_insights
                    if strategy_trajs[best_idx][1] is not None and "mistake_insights" in strategy_trajs[best_idx][1]:
                        refined_insights = strategy_trajs[best_idx][1]["mistake_insights"]
                    elif use_mistakes and 'mistake_insights' in item:
                        refined_insights = item['mistake_insights']
                    else:
                        refined_insights = ""
                else:
                    # Fallback: generate without memory
                    refined_insights = item.get('mistake_insights', "") if use_mistakes else ""
                ...
                while cur_iter < max_iters:
                    ...
                    while ref_id < min(len(div_reflections), 2):
                        ...
                        retrieval_pool = global_positive_trajectories + memory_bank["positive_trajectories"]
                        # ENSEMBLE: Use feedback-guided for reflection rounds
                        closest_ref_traj = select_stage2_traj_ensemble(
                            retrieval_pool=retrieval_pool,
                            curr_prompt_emb=curr_emb,
                            reflection_text=reflection,
                            feedback_history=test_feedback,
                            strategy="feedback_guided",
                        )
                        router_mix = [0.0, 0.0, 0.0]
                        router_conf = 0.0
                        fallback_flag = True
