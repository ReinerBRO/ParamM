...
def together_chat(
    ...
    # Call Together chat API. Relay fallback is intentionally disabled for pipeline consistency.
    request_model = model if use_together else _relay_model_alias(model)
    fallback_model = os.environ.get("RELAY_FALLBACK_MODEL", "").strip()
    if (not use_together) and fallback_model:
        raise RuntimeError(
            "RELAY_FALLBACK_MODEL must be empty in strict mode; relay fallback is disabled."
        )
    candidate_models = [request_model]

    response = None
    last_err = None
    for request_model_i in candidate_models:
        try:
            response = client.chat.completions.create(
                model=request_model_i,
            ...
            if request_model_i != request_model:
                print(f"[warn] relay fallback model used: {request_model_i} (primary={request_model})")
            break
        except Exception as e:
            last_err = e
            print(f"[warn] model request failed ({request_model_i}): {e}")
            continue
    if response is None:
