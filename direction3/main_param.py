...
def get_args():
    ...
    parser.add_argument("--inner_iter", type=int,
                        help="inner ierations", default=5)
    ...
    parser.add_argument("--router_enable", action='store_true',
                        help="Enable online router-guided retrieval in DoT stage2")
    parser.add_argument("--router_conf_threshold", type=float, default=0.6,
...
def strategy_factory(strategy: str):
    ...
    if strategy == "simple":
        return kwargs_wrapper_gen(run_simple, delete_keys=["expansion_factor", "phase1_only", "global_mem_bank_path", "router_enable", "router_conf_threshold", "router_ckpt_path"])
    elif strategy == "dot":
        return kwargs_wrapper_gen(run_dot, delete_keys=["expansion_factor"])
    elif strategy == "dot_bank":
        return kwargs_wrapper_gen(run_dot_bank, delete_keys=["expansion_factor", "phase1_only", "global_mem_bank_path", "router_enable", "router_conf_threshold", "router_ckpt_path"])
    elif strategy == "reflexion":
        return kwargs_wrapper_gen(run_reflexion, delete_keys=["expansion_factor", "phase1_only", "global_mem_bank_path", "router_enable", "router_conf_threshold", "router_ckpt_path"])
    elif strategy == "test-acc":
        return kwargs_wrapper_gen(run_test_acc, delete_keys=["expansion_factor", "max_iters", "phase1_only", "global_mem_bank_path", "router_enable", "router_conf_threshold", "router_ckpt_path"])
    else:
...
""")
    ...
    if args.strategy != 'simple' and 'mbpp' not in args.dataset_path.lower():
        run_strategy(
            ...
            router_enable=args.router_enable,
            router_conf_threshold=args.router_conf_threshold,
