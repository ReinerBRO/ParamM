...
def _normalized_entropy(weights: List[float]) -> float:
    vals = [max(0.0, float(v)) for v in (weights or [])]
    if len(vals) == 0:
        return 1.0
    total = sum(vals)
    if total <= 1e-12:
    ...
    if len(probs) <= 1:
        return 0.0
    entropy = -sum(p * float(np.log(p)) for p in probs)
...
def _top1_top2_margin(scores: List[float]) -> float:
    if not scores:
        return 0.0
    if len(scores) == 1:
        return 1.0
    top2 = sorted((float(s) for s in scores), reverse=True)[:2]
    return float(top2[0] - top2[1])


def _rank_of_index(scores: List[float], idx: int) -> int:
    if not scores:
        return 0
    ranked = sorted(
    ...
    for rank, (_, i) in enumerate(ranked, start=1):
        if i == idx:
            return rank
    return len(scores)


def _calibrated_router_accept(
    router_conf: float,
    router_mix: List[float],
    fused_scores: List[float],
    conf_threshold: float,
    min_margin: float = 0.05,
    max_entropy: float = 0.95,
) -> Tuple[bool, Dict[str, float]]:
    entropy = _normalized_entropy(router_mix)
    margin = _top1_top2_margin(fused_scores)
    conf_ok = float(router_conf) >= float(conf_threshold)
    margin_ok = margin >= float(min_margin)
    entropy_ok = entropy <= float(max_entropy)
    # Hotfix: gate off margin first because confidence is under-discriminative.
    accept = bool(margin_ok)
    return accept, {
        "router_entropy": float(entropy),
        "top1_top2_margin": float(margin),
        "gate_conf_ok": 1.0 if conf_ok else 0.0,
        "gate_margin_ok": 1.0 if margin_ok else 0.0,
        "gate_entropy_ok": 1.0 if entropy_ok else 0.0,
    }


def _apply_prompt_dominance_clamp(
    router_mix: List[float],
    min_prompt_weight: float = 0.55,
) -> List[float]:
    vals = [max(0.0, float(v)) for v in (router_mix or [])[:3]]
    while len(vals) < 3:
        vals.append(0.0)
    ...
    min_prompt_weight = max(0.0, min(1.0, float(min_prompt_weight)))
    if vals[0] < min_prompt_weight:
        remaining = vals[1] + vals[2]
        target_remaining = max(0.0, 1.0 - min_prompt_weight)
        if remaining <= 1e-12:
            vals[1] = target_remaining * 0.5
            vals[2] = target_remaining * 0.5
        else:
            scale = target_remaining / remaining
            vals[1] *= scale
            vals[2] *= scale
        vals[0] = min_prompt_weight
...
def _fallback_blend_alpha(
    router_conf: float,
    conf_threshold: float,
    rejected: bool,
    reject_due_to_margin: bool,
    reject_alpha_cap: float = 0.35,
) -> float:
    threshold = max(float(conf_threshold), 1e-6)
    calibrated_alpha = max(0.0, min(1.0, float(router_conf) / threshold))
    if not rejected:
        return calibrated_alpha
    if reject_due_to_margin:
        return 0.0
    return min(float(reject_alpha_cap), calibrated_alpha)
...
def run_dot(
    ...
    for log_item in logs_copy:
        ...
        log_item.setdefault("router_entropy", 1.0)
        log_item.setdefault("top1_top2_margin", 0.0)
        log_item.setdefault("chosen_candidate_rank", 0)
    ...
    def select_stage2_traj(
    ...
    ) -> Tuple[Optional[dict], List[float], float, bool, Dict[str, float]]:
        if len(retrieval_pool) == 0:
            return None, [0.0, 0.0, 0.0], 0.0, True, {
                "router_entropy": 1.0,
                "top1_top2_margin": 0.0,
                "chosen_candidate_rank": 0.0,
            }
        ...
        try:
            ...
            negative_penalties: List[float] = []
            has_negative_emb = any(neg.get("prompt_embedding") is not None for neg in negative_pool)
            for traj in candidates:
                traj_emb = traj.get("prompt_embedding")
                cand_penalty = 0.0
                if traj_emb is not None and has_negative_emb:
                    cand_penalty = max(
            ...
            reflection_available = reflection_query is not None and any(
            ...
            if not shortlist_indices:
                shortlist_indices = list(range(len(candidates)))
            shortlist_candidates = [candidates[idx] for idx in shortlist_indices]
            shortlist_prompt_sims = [prompt_sims[idx] for idx in shortlist_indices]
            shortlist_reflection_sims = [reflection_sims[idx] for idx in shortlist_indices]
            shortlist_negative_penalties = [negative_penalties[idx] for idx in shortlist_indices]
            ...
            router_state = {
                ...
                "retrieval_candidate_count": len(shortlist_candidates),
            }
            ...
            router_output = infer_transformer_router(
                ...
                prompt_sims=shortlist_prompt_sims,
                reflection_sims=shortlist_reflection_sims,
                negative_penalties=shortlist_negative_penalties,
            )
            ...
            w0, w1, w2 = _apply_prompt_dominance_clamp(router_mix, min_prompt_weight=0.55)

            fused_scores = [
                (w0 * shortlist_prompt_sims[idx])
                + (w1 * shortlist_reflection_sims[idx])
                - (w2 * shortlist_negative_penalties[idx])
                for idx in range(len(shortlist_candidates))
            ]

            accept_router, diag = _calibrated_router_accept(
                router_conf=router_conf,
                router_mix=router_mix,
                fused_scores=fused_scores,
                conf_threshold=router_conf_threshold,
            )

            if accept_router:
                best_idx = int(np.argmax(np.asarray(fused_scores)))
                diag["chosen_candidate_rank"] = float(_rank_of_index(fused_scores, best_idx))
                diag["fallback_alpha"] = 1.0
                return shortlist_candidates[best_idx], [w0, w1, w2], router_conf, False, diag

            reject_due_to_margin = bool(diag.get("gate_margin_ok", 0.0) < 0.5)
            fallback_scores = shortlist_prompt_sims if reject_due_to_margin else (
                shortlist_reflection_sims if reflection_available else shortlist_prompt_sims
            )
            alpha = _fallback_blend_alpha(
                router_conf=router_conf,
                conf_threshold=router_conf_threshold,
                rejected=True,
                reject_due_to_margin=reject_due_to_margin,
            )
            blended_scores = [
            ...
            diag["chosen_candidate_rank"] = float(_rank_of_index(blended_scores, best_idx))
            diag["fallback_alpha"] = float(alpha)
            return shortlist_candidates[best_idx], [w0, w1, w2], router_conf, True, diag
        except Exception:
            return fallback_traj, [0.0, 0.0, 0.0], 0.0, True, {
                "router_entropy": 1.0,
                "top1_top2_margin": 0.0,
    ...
    for i, item in enumerate(failed_problems):
        ...
        try:
            ...
            while cur_pass < pass_at_k and not is_solved:
                ...
                initial_traj, router_mix, router_conf, fallback_flag, router_diag = select_stage2_traj(
                ...
                router_entropy = float(router_diag.get("router_entropy", 1.0))
                top1_top2_margin = float(router_diag.get("top1_top2_margin", 0.0))
                chosen_candidate_rank = int(router_diag.get("chosen_candidate_rank", 0))
                ...
                while cur_iter < max_iters:
                    ...
                    while ref_id < min(len(div_reflections), 2):
                        ...
                        retrieval_pool = global_positive_trajectories + memory_bank["positive_trajectories"]
                        closest_ref_traj, router_mix, router_conf, fallback_flag, router_diag = select_stage2_traj(
                            retrieval_pool=retrieval_pool,
                            curr_prompt_emb=curr_emb,
                        ...
                        router_entropy = float(router_diag.get("router_entropy", router_entropy))
                        top1_top2_margin = float(router_diag.get("top1_top2_margin", top1_top2_margin))
                        chosen_candidate_rank = int(router_diag.get("chosen_candidate_rank", chosen_candidate_rank))
        ...
        for log_item in logs_copy:
            if log_item.get(primary_key) == item.get(primary_key):
                ...
                log_item["router_entropy"] = float(router_entropy)
                log_item["top1_top2_margin"] = float(top1_top2_margin)
                log_item["chosen_candidate_rank"] = int(chosen_candidate_rank)
